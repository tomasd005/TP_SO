...
echo "=================================================="
echo "==== Iniciando Experiência: Paralelismo (N=4) ===="
echo "====   Configuração: 4 slots, Política: rr    ===="
echo "=================================================="
...
read -p "Por favor, inicie o controller noutro terminal com: ./bin/controller 4 rr e pressione [ENTER] para continuar..."
...
for i in {1..8}; do
    if [ "$i" -eq 4 ]; then
        ./bin/runner -e 1 "sleep 1 | grep 'teste' > tmp/out_parallel.txt" > /dev/null 2>&1 &
    else
